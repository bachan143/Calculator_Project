  ...
  public class MyCalculator extends Frame implements ActionListener
{
	...
	public  MyCalculator()
	{
		...
		add(one);add(two);add(three);add(four);add(five);add(six);add(seven);add(eight);add(nine);add(zero);add(dot);add(clear);
		add(sub);add(mul);add(div);add(EQ);add(sqrt);add(add);add(tf1);add(mod);
		add(tf1);
